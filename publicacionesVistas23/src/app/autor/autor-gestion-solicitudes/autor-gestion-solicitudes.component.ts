...
export class AutorGestionSolicitudesComponent implements OnInit {
  ...
  //INSTANCIA VARIABLES
  instanciaVariables() {
    this.vecArticulo = []; this.vecPersonaArticulo = []; this.numRegistro = 0; this.vecArtCongreso = []; this.vecLibro = []; this.vecBuscaAutores = []; this.vecModDocumento = [];
    ...
    this.vecModDocumento.push({ opcion: 0, distributivo: 0, ruta1: 0, ruta2: 0, ruta3: 0 })
  }
  ...
  ingresoLibro() {
    this.swPublicacion.postAddUsuario(this.numRegistro, '', this.vecLibro[0]['isbn'], 1, 0, 0, this.generaRutas(9, this.numRegistro), this.generaRutas(7, this.numRegistro),
      'na', 'na', 'na', 'na', 'na', 'na', 'na', 17).subscribe((data: any) => {

        this.tokenDrive(this.documento06, this.generaRutas(7, this.numRegistro));
        this.tokenDrive(this.documento07, this.generaRutas(9, this.numRegistro));
        this.verCargasArticulos();
        this.mr.close();
  ...
  async cargaDrive(archivo: any, token: any, raiz: string) {
    ...
    this.swPublicacion.subirArchivos(formArchivo, token).subscribe(data => {
      if (data.success)
        this.alerti.success('Archivo Actualizado');
      else
        this.alerti.error('Error inesperad, vuelva a intentar');
    });
  }
  //SANITIZA EL BASE 64
  ...
  generaRutas(op: number, numero: string) {
    ...
    if (op == 9)
      return ("publicaciones/" + year + "/libros/informe/" + localStorage.getItem('loginCedula') + "_informe_" + numero + ".pdf");
    else
      return '';
  }
  ...
  //VER MODAL PARA EDITAR DOCUMENTOS
  verModDocumentos(objPublica: any, nombModal: any) {
    this.instanciaVariables();
    this.mr = this.modalService.open(nombModal);
    this.vecModDocumento[0]['opcion'] = objPublica.tipoPublica;
    this.vecModDocumento[0]['distributivo'] = objPublica.distributivo;
    this.vecModDocumento[0]['ruta1'] = objPublica.ruta1;
    this.vecModDocumento[0]['ruta2'] = objPublica.ruta2;
    this.vecModDocumento[0]['ruta3'] = objPublica.ruta3;
  }
